...
datasources = [plot.draw_points_with_circle_glyph([], { 'fill_color' : color_map[i],
...
def update_plot(day):
	...
	for k, g in groupby(transactions, lambda t: t.pole):
		...
		i = min(floor(HEATMAP_SCALING * hours / Transaction.MAX_DURATION * HEATMAP_GRANULARITY), HEATMAP_GRANULARITY - 1)
		poles_by_heat[i] += [k]
